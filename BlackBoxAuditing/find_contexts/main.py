...
parser.add_argument("original_train_csv", type=str,
                   help="The training set of the data for which contexts of influence are discovered.")
parser.add_argument("original_test_csv", type=str,
                   help="The test set of the data for which contexts of influence are discovered.")
parser.add_argument("obscured_test_csv", type=str,
...
parser.add_argument("epsilon", type=float, default=0.05,
                   help="Number within which we consider best expanded rule of the original quality.")
...
original_train_tab = convert_to_tab(args.original_train_csv, args.feature_data_file, output_dir)
original_test_tab = convert_to_tab(args.original_test_csv, args.feature_data_file, output_dir)
merged_csv = merge(args.original_test_csv, args.obscured_test_csv, obscured_tag, output_dir)

# Generate rule list for the original data using the CN2 algorithm
rulesfile, accuracy, AUC = CN2_learner(original_train_tab, original_test_tab, output_dir, args.beam_width, args.min_covered_examples, args.max_rule_length, influence_scores)
...
contexts_of_influence = expand_and_find_contexts(args.original_test_csv, args.obscured_test_csv, merged_csv, rulesfile, influence_scores, obscured_tag,output_dir, args.by_original, args.epsilon)
